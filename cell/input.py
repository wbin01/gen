...
class Input(Cell):
    ...
    def _draw(self, mode: str = None) -> None:
        ...
        if mode == 'UNIT':
            if self._invalidate_item == 'SELECT':
                ...
                self._invalidate_item = None
                self._text_tx, self._textw, self._texth = self._drawer.font(
        ...
        if mode == 'REBUILD':
            ...
            self._tx_w = int(self.width)
            self._tx_h = int(self.height)

            self._tx_default = self._drawer.build_texture(
                self._tx_w, self._tx_h, self._draw_obj, 'BASE')
            
            self._tx_hover = self._drawer.build_texture(
                self._tx_w, self._tx_h, self._draw_obj, 'HOVER')
            
            self._tx_pressed = self._drawer.build_texture(
                self._tx_w, self._tx_h, self._draw_obj, 'PRESSED')
            ...
            self._cursor_tx_on = self._drawer.build_texture(
                self._cursor_w, font_size, self._draw_obj, 'CURSOR_ON')
            
            self._cursor_tx_off = self._drawer.build_texture(
    ...
    def _draw_obj(self, state: str = 'BASE'):
